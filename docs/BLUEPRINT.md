# KAI ERP Connector

## Complete Technical Blueprint

### From SyteLine Data to AI-Ready Tools



```
╔═══════════════════════════════════════════════════════════════════════════════╗
║                                                                               ║
║   "Hey, what's the welding department working on today?"                      ║
║                                                                               ║
║                              ↓                                                ║
║                                                                               ║
║   "WELD-01 has 3 jobs in progress. J-1234 is 45% complete and on schedule.   ║
║    J-1235 is behind - only 15% done but was due yesterday. You might want    ║
║    to check on that one."                                                     ║
║                                                                               ║
╚═══════════════════════════════════════════════════════════════════════════════╝
```



**Prepared for:** Kaspar Companies  
**Date:** December 2024  
**Version:** 3.0 (Final)



---



# Table of Contents



1. [Executive Summary](#1-executive-summary)
2. [The Complete Picture](#2-the-complete-picture)
3. [Architecture Deep Dive](#3-architecture-deep-dive)
4. [Data Source Layer](#4-data-source-layer)
5. [Connector Layer](#5-connector-layer)
6. [MCP Server Layer](#6-mcp-server-layer)
7. [Bedrock Ops Scheduler](#7-bedrock-ops-scheduler-flagship-connector)
8. [Implementation Roadmap](#8-implementation-roadmap)
9. [Appendices](#9-appendices)



---



# 1. Executive Summary



## The Challenge



Kaspar Companies migrated from SyteLine 8 (on-premises) to SyteLine 10 CloudSuite. This eliminated direct SQL database access that powered production dashboards, operational tools, and reporting across Bedrock Truck Beds, Horizon Firearms, and Circle Y Saddles.



## The Solution



The **KAI ERP Connector** is a three-layer system that:



```
┌─────────────────────────────────────────────────────────────────────────┐
│                                                                         │
│   LAYER 3: MCP SERVER                              ← AI talks here      │
│   Tools that AI agents can discover and use                             │
│                                                                         │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   LAYER 2: CONNECTORS                              ← Business logic     │
│   Bedrock Scheduler, Sales Orders, Inventory, etc.                      │
│                                                                         │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   LAYER 1: DATA SOURCES                            ← Complexity hidden  │
│   REST APIs (real-time) + Data Lake (bulk/history)                      │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```



## Key Outcomes



| Capability | Before (SL8) | After Migration | With KAI Connector |
|------------|--------------|-----------------|-------------------|
| Real-time production data | ✅ Direct SQL | ❌ Lost | ✅ Restored |
| AI agent access | ❌ None | ❌ None | ✅ Full |
| Query complexity | 1 SQL query | 6+ API calls | 1 tool call |
| Developer experience | Write SQL | Learn Infor APIs | Call endpoint |



## Investment Summary



| Phase | Duration | Deliverable |
|-------|----------|-------------|
| Phase 1 | 2 weeks | Core infrastructure + Bedrock Scheduler |
| Phase 2 | 2 weeks | Additional connectors + Data Lake integration |
| Phase 3 | 1 week | MCP Server + AI agent integration |
| **Total** | **5 weeks** | **Complete AI-ready ERP access** |



---



# 2. The Complete Picture



## 2.1 System Overview



```
╔═════════════════════════════════════════════════════════════════════════════════╗
║                                                                                 ║
║                              CONSUMERS                                          ║
║                                                                                 ║
║    ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐               ║
║    │  Claude  │    │ Bedrock  │    │  Custom  │    │  Future  │               ║
║    │    AI    │    │Dashboard │    │  Tools   │    │   Apps   │               ║
║    │  Agent   │    │          │    │          │    │          │               ║
║    └────┬─────┘    └────┬─────┘    └────┬─────┘    └────┬─────┘               ║
║         │               │               │               │                      ║
║         │   MCP         │   REST        │   REST        │                      ║
║         │   Protocol    │   API         │   API         │                      ║
║         ▼               ▼               ▼               ▼                      ║
║    ┌─────────────────────────────────────────────────────────────────────┐    ║
║    │                                                                     │    ║
║    │                      KAI ERP CONNECTOR                              │    ║
║    │                                                                     │    ║
║    │  ┌─────────────────────────────────────────────────────────────┐   │    ║
║    │  │                     MCP SERVER                              │   │    ║
║    │  │                                                             │   │    ║
║    │  │   Tools:                                                    │   │    ║
║    │  │   • get_production_schedule                                 │   │    ║
║    │  │   • get_open_orders                                         │   │    ║
║    │  │   • search_customers                                        │   │    ║
║    │  │   • get_inventory_status                                    │   │    ║
║    │  │   • get_job_details                                         │   │    ║
║    │  │                                                             │   │    ║
║    │  └──────────────────────────┬──────────────────────────────────┘   │    ║
║    │                             │                                       │    ║
║    │  ┌──────────────────────────▼──────────────────────────────────┐   │    ║
║    │  │                    CONNECTORS                               │   │    ║
║    │  │                                                             │   │    ║
║    │  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐           │   │    ║
║    │  │  │   Bedrock   │ │    Sales    │ │  Inventory  │    ...    │   │    ║
║    │  │  │  Ops Sched  │ │   Orders    │ │   Status    │           │   │    ║
║    │  │  └─────────────┘ └─────────────┘ └─────────────┘           │   │    ║
║    │  │                                                             │   │    ║
║    │  └──────────────────────────┬──────────────────────────────────┘   │    ║
║    │                             │                                       │    ║
║    │  ┌──────────────────────────▼──────────────────────────────────┐   │    ║
║    │  │                   QUERY ROUTER                              │   │    ║
║    │  │                                                             │   │    ║
║    │  │   < 5K records + real-time needed? → REST APIs              │   │    ║
║    │  │   > 10K records or history?        → Data Lake              │   │    ║
║    │  │                                                             │   │    ║
║    │  └───────────────┬─────────────────────────┬────────────────────┘   │    ║
║    │                  │                         │                        │    ║
║    │                  ▼                         ▼                        │    ║
║    │  ┌────────────────────────┐  ┌────────────────────────────┐        │    ║
║    │  │    REST API ENGINE     │  │    DATA LAKE ENGINE        │        │    ║
║    │  │                        │  │                            │        │    ║
║    │  │  • Token management    │  │  • Compass SQL             │        │    ║
║    │  │  • Parallel fetch      │  │  • Bulk extraction         │        │    ║
║    │  │  • DuckDB staging      │  │  • Historical queries      │        │    ║
║    │  │  • Client-side joins   │  │  • Server-side joins       │        │    ║
║    │  │                        │  │                            │        │    ║
║    │  └───────────┬────────────┘  └──────────────┬─────────────┘        │    ║
║    │              │                              │                       │    ║
║    └──────────────┼──────────────────────────────┼───────────────────────┘    ║
║                   │                              │                            ║
╚═══════════════════┼══════════════════════════════┼════════════════════════════╝
                    │                              │
                    ▼                              ▼
         ┌───────────────────┐          ┌───────────────────┐
         │                   │          │                   │
         │  SYTELINE 10      │          │   INFOR DATA      │
         │  CLOUD            │          │   LAKE            │
         │                   │          │                   │
         │  IDO REST APIs    │          │   Compass SQL     │
         │  (Real-time)      │          │   (10-15 min lag) │
         │                   │          │                   │
         └───────────────────┘          └───────────────────┘
```



## 2.2 Data Flow Example



**User asks:** "What's running in welding today?"



```
┌─────────────────────────────────────────────────────────────────────────────┐
│ STEP 1: AI RECEIVES QUESTION                                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   User: "What's running in welding today?"                                  │
│                                                                             │
│   AI thinks:                                                                │
│   • This is about production/manufacturing                                  │
│   • I have a tool called get_production_schedule                            │
│   • Welding = WELD-01 or WELD-02 work centers                               │
│   • I should call that tool                                                 │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ STEP 2: MCP TOOL CALL                                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   {                                                                         │
│     "tool": "get_production_schedule",                                      │
│     "arguments": {                                                          │
│       "work_center": "WELD-01"                                              │
│     }                                                                       │
│   }                                                                         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ STEP 3: CONNECTOR EXECUTES                                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   BedrockOpsScheduler:                                                      │
│                                                                             │
│   1. Estimate volume: ~50 operations (small)                                │
│   2. Freshness: Real-time needed                                            │
│   3. Router decision: Use REST APIs                                         │
│                                                                             │
│   4. Parallel fetch 6 IDOs:                                                 │
│      ┌────────────┐ ┌────────────┐ ┌────────────┐                          │
│      │  SLJobs    │ │SLJobroutes │ │ SLJrtSchs  │                          │
│      │   ~200ms   │ │   ~200ms   │ │   ~200ms   │                          │
│      └────────────┘ └────────────┘ └────────────┘                          │
│      ┌────────────┐ ┌────────────┐ ┌────────────┐                          │
│      │  SLItems   │ │SLItemwhses │ │   SLWcs    │                          │
│      │   ~150ms   │ │   ~150ms   │ │   ~100ms   │                          │
│      └────────────┘ └────────────┘ └────────────┘                          │
│                                                                             │
│      Total time: ~400ms (parallel, slowest wins)                            │
│                                                                             │
│   5. Stage in DuckDB, execute JOIN                                          │
│   6. Return structured operations                                           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ STEP 4: STRUCTURED RESPONSE                                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   {                                                                         │
│     "operations": [                                                         │
│       {                                                                     │
│         "job": "J-1234",                                                    │
│         "item": "BED-KING-BLK",                                             │
│         "item_description": "King Bed Frame - Black",                       │
│         "work_center": "WELD-01",                                           │
│         "percent_complete": 45.0,                                           │
│         "schedule": {"start": "06:00", "finish": "14:00"},                  │
│         "status": "on_track"                                                │
│       },                                                                    │
│       {                                                                     │
│         "job": "J-1235",                                                    │
│         "item": "BED-QUEEN-WHT",                                            │
│         "percent_complete": 15.0,                                           │
│         "schedule": {"start": "yesterday", "finish": "yesterday"},          │
│         "status": "behind"                                                  │
│       }                                                                     │
│     ],                                                                      │
│     "summary": {"total": 2, "on_track": 1, "behind": 1}                     │
│   }                                                                         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ STEP 5: AI RESPONDS                                                         │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   "WELD-01 has 2 jobs running today:                                        │
│                                                                             │
│    • J-1234 (King Bed Frame - Black) is 45% complete and on track           │
│      to finish by 2 PM                                                      │
│                                                                             │
│    • J-1235 (Queen Bed Frame - White) is behind schedule - only 15%         │
│      done but was due yesterday. You might want to check on that one."      │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



---



# 3. Architecture Deep Dive



## 3.1 The Three-Layer Stack



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│  LAYER 3: MCP SERVER                                                        │
│  ═══════════════════                                                        │
│                                                                             │
│  PURPOSE: Make connectors accessible to AI                                  │
│                                                                             │
│  WHAT IT DOES:                                                              │
│  • Publishes tool catalog (AI discovers what's available)                   │
│  • Defines parameters with descriptions (AI knows what to pass)             │
│  • Validates inputs (catches errors before execution)                       │
│  • Translates errors (AI-friendly messages)                                 │
│                                                                             │
│  AI SEES:                              AI DOESN'T SEE:                      │
│  ✓ Tool names                          ✗ REST API details                   │
│  ✓ Parameter schemas                   ✗ Authentication                     │
│  ✓ Descriptions of when to use         ✗ Which IDOs to call                 │
│  ✓ Structured responses                ✗ Join logic                         │
│                                        ✗ REST vs Data Lake routing          │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  LAYER 2: CONNECTORS                                                        │
│  ═══════════════════                                                        │
│                                                                             │
│  PURPOSE: Encapsulate business logic for each data access pattern           │
│                                                                             │
│  WHAT IT DOES:                                                              │
│  • Defines which IDOs/tables are needed                                     │
│  • Specifies join relationships                                             │
│  • Calculates derived fields (percent complete, status, etc.)               │
│  • Estimates volume for routing decisions                                   │
│  • Maps raw data to business objects                                        │
│                                                                             │
│  EACH CONNECTOR DEFINES:               CONNECTORS DON'T HANDLE:             │
│  ✓ REST query spec (IDOs, props)       ✗ Token management                   │
│  ✓ Data Lake query (Compass SQL)       ✗ HTTP requests                      │
│  ✓ Join logic (DuckDB SQL)             ✗ Error retry logic                  │
│  ✓ Output model (Pydantic)             ✗ Parallel execution                 │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  LAYER 1: DATA SOURCES                                                      │
│  ═════════════════════                                                      │
│                                                                             │
│  PURPOSE: Handle raw data access complexity                                 │
│                                                                             │
│  REST API ENGINE:                      DATA LAKE ENGINE:                    │
│  • OAuth token management              • ION API authentication             │
│  • Parallel fetch (asyncio)            • Compass SQL execution              │
│  • Rate limit handling                 • Large result pagination            │
│  • DuckDB staging & joins              • Query timeout handling             │
│  • Retry with backoff                  • Sync timestamp tracking            │
│                                                                             │
│  BEST FOR:                             BEST FOR:                            │
│  • Real-time data                      • Historical analysis                │
│  • < 5,000 records                     • > 10,000 records                   │
│  • Operational queries                 • Trend reporting                    │
│  • AI agent lookups                    • Bulk extracts                      │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



## 3.2 Why Three Layers?



| Layer | Changes When... | Example |
|-------|----------------|---------|
| **MCP Server** | AI interface needs to evolve | Add new parameter to tool |
| **Connectors** | Business logic changes | New calculated field |
| **Data Sources** | Infor updates APIs | New authentication method |



**Key insight:** Each layer can change independently. Swap REST for Data Lake? Connectors don't change. Add new tool parameter? Data sources don't change.



---



# 4. Data Source Layer



## 4.1 REST API Engine



### Volume Thresholds



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│                         REST API VOLUME GUIDE                               │
│                                                                             │
│   Records    │ Status      │ Action                                         │
│   ───────────┼─────────────┼──────────────────────────────────────────────  │
│   < 500      │ ✅ OPTIMAL  │ Single request, fast response                  │
│   ───────────┼─────────────┼──────────────────────────────────────────────  │
│   500-2000   │ ✅ GOOD     │ Single request, acceptable latency             │
│   ───────────┼─────────────┼──────────────────────────────────────────────  │
│   2000-5000  │ ⚠️ CAUTION  │ May need pagination, monitor latency          │
│   ───────────┼─────────────┼──────────────────────────────────────────────  │
│   5000-10000 │ ⚠️ EVALUATE │ Consider Data Lake if freshness allows        │
│   ───────────┼─────────────┼──────────────────────────────────────────────  │
│   > 10000    │ ❌ TOO MUCH │ Use Data Lake - REST not designed for this    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



### Parallel Fetch Architecture



```python
class RestEngine:
    """
    Fetches from multiple IDOs simultaneously.
    
    Why parallel?
    - Sequential: 6 calls × 400ms = 2.4 seconds
    - Parallel:   6 calls in parallel = ~500ms (slowest wins)
    """
    
    async def parallel_fetch(self, ido_specs: list[IDOSpec]) -> dict:
        # Create all fetch tasks
        tasks = [self._fetch_ido(spec) for spec in ido_specs]
        
        # Execute simultaneously
        results = await asyncio.gather(*tasks)
        
        return dict(zip([s.name for s in ido_specs], results))
```



```
PARALLEL FETCH VISUALIZATION:

Time →  0ms    100ms   200ms   300ms   400ms   500ms
        │       │       │       │       │       │
SLJobs  ████████████████████████████████░░░░░░░░  (400ms)
        │       │       │       │       │       │
SLJobrt ██████████████████████████████████████░░  (450ms) ← Slowest
        │       │       │       │       │       │
SLJrtSc █████████████████████████████░░░░░░░░░░░  (350ms)
        │       │       │       │       │       │
SLItems ████████████████████░░░░░░░░░░░░░░░░░░░░  (250ms)
        │       │       │       │       │       │
SLItemW █████████████████████████░░░░░░░░░░░░░░░  (300ms)
        │       │       │       │       │       │
SLWcs   ██████████████░░░░░░░░░░░░░░░░░░░░░░░░░░  (200ms)
        │       │       │       │       │       │
        └───────┴───────┴───────┴───────┴───────┘
                                                │
                                    All data ready: 450ms
```



### DuckDB Staging Layer



```
WHY DUCKDB?

┌────────────────────────────────────────────────────────────────────────────┐
│                                                                            │
│  The Problem: SyteLine returns data from separate IDOs                     │
│                                                                            │
│  SLJobs returns:        SLJobroutes returns:     SLItems returns:          │
│  ┌─────────────────┐    ┌──────────────────┐     ┌─────────────────┐       │
│  │ Job  │ Item     │    │ Job │ Wc   │ Qty │     │ Item  │ Desc    │       │
│  │──────┼──────────│    │─────┼──────┼─────│     │───────┼─────────│       │
│  │ 1234 │ BED-KING │    │1234 │WELD-1│ 50  │     │BED-KNG│King Bed │       │
│  │ 1235 │ BED-QUEN │    │1234 │PAINT │ 50  │     │BED-QN │Queen Bed│       │
│  └─────────────────┘    │1235 │WELD-1│ 30  │     └─────────────────┘       │
│                         └──────────────────┘                               │
│                                                                            │
│  We need to JOIN these to get usable data                                  │
│                                                                            │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                            │
│  The Solution: Stage in DuckDB, join with SQL                              │
│                                                                            │
│  1. Load each IDO response into a DuckDB table                             │
│  2. Execute JOIN query (same SQL you'd write for direct DB access)         │
│  3. Return combined results                                                │
│  4. DuckDB instance is discarded (in-memory, ephemeral)                    │
│                                                                            │
│  ┌──────────────────────────────────────────────────────────────────┐      │
│  │                                                                  │      │
│  │  SELECT j.Job, j.Item, i.Description, jr.Wc, jr.Qty             │      │
│  │  FROM jobs j                                                     │      │
│  │  JOIN jobroutes jr ON j.Job = jr.Job                            │      │
│  │  JOIN items i ON j.Item = i.Item                                │      │
│  │                                                                  │      │
│  └──────────────────────────────────────────────────────────────────┘      │
│                                                                            │
│  Result:                                                                   │
│  ┌─────────────────────────────────────────────────────────┐               │
│  │ Job  │ Item     │ Description │ Wc     │ Qty │          │               │
│  │──────┼──────────┼─────────────┼────────┼─────│          │               │
│  │ 1234 │ BED-KING │ King Bed    │ WELD-1 │ 50  │  ← Ready │               │
│  │ 1234 │ BED-KING │ King Bed    │ PAINT  │ 50  │    to    │               │
│  │ 1235 │ BED-QUEN │ Queen Bed   │ WELD-1 │ 30  │    use!  │               │
│  └─────────────────────────────────────────────────────────┘               │
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
```



## 4.2 Data Lake Engine



### When to Use Data Lake



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│                    DATA SOURCE DECISION MATRIX                              │
│                                                                             │
│                                                                             │
│                         DATA FRESHNESS REQUIREMENT                          │
│                                                                             │
│                    Real-time      Near-RT       Batch OK                    │
│                    (seconds)     (minutes)      (hours)                     │
│                         │            │             │                        │
│         ┌───────────────┼────────────┼─────────────┼──────────────┐        │
│         │               │            │             │              │        │
│  < 500  │   REST API    │  REST API  │   Either   │              │        │
│         │      ✅        │     ✅      │     ✅      │              │        │
│         │               │            │             │              │        │
│ VOLUME  ├───────────────┼────────────┼─────────────┤              │        │
│         │               │            │             │              │        │
│  500-   │   REST API    │  REST API  │  Data Lake │              │        │
│  5000   │      ✅        │     ✅      │     ✅      │              │        │
│         │               │            │             │              │        │
│         ├───────────────┼────────────┼─────────────┤              │        │
│         │               │            │             │              │        │
│  5K-    │   Evaluate    │ Data Lake  │  Data Lake │              │        │
│  10K    │      ⚠️        │     ✅      │     ✅      │              │        │
│         │               │            │             │              │        │
│         ├───────────────┼────────────┼─────────────┤              │        │
│         │               │            │             │              │        │
│  > 10K  │   Stream*     │ Data Lake  │  Data Lake │              │        │
│         │      💰        │     ✅      │     ✅      │              │        │
│         │               │            │             │              │        │
│         └───────────────┴────────────┴─────────────┘              │        │
│                                                                             │
│  * Stream Pipelines requires additional licensing                           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



### Data Lake Limitations



```
⚠️  IMPORTANT: DATA LAKE CONSTRAINTS

┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│  1. SYNC DELAY: 10-15 minutes minimum                                       │
│     Data Lake is NOT real-time. A job completed 5 minutes ago               │
│     may not appear yet.                                                     │
│                                                                             │
│  2. LIMITED TABLES: Not everything replicates                               │
│     Only tables in DATALAKE replication category are available.             │
│     Custom tables may not be included.                                      │
│                                                                             │
│  3. ADDITIONAL LICENSING: Not included in base CloudSuite                   │
│     Data Fabric add-on required. Stream Pipelines costs extra.              │
│                                                                             │
│  4. QUERY LIMITS: 60-minute timeout, 100K rows per page                     │
│     Very complex queries may fail.                                          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘

RECOMMENDATION: Start with REST APIs (included, real-time, sufficient for 
most operational use cases). Add Data Lake when specific analytical 
requirements demand it.
```



---



# 5. Connector Layer



## 5.1 Connector Pattern



Each connector is a self-contained module that defines a complete data access pattern:



```python
class BaseConnector(ABC):
    """
    Base class for all KAI ERP Connectors.
    
    Each connector encapsulates:
    - Which data sources to fetch
    - How to join/transform the data
    - What business object to return
    """
    
    @abstractmethod
    def get_rest_spec(self) -> RestQuerySpec:
        """Define REST API access pattern."""
        pass
    
    @abstractmethod
    def get_lake_query(self) -> str:
        """Define Data Lake SQL query."""
        pass
    
    @abstractmethod
    def estimate_volume(self, filters: dict) -> int:
        """Estimate result count for routing."""
        pass
    
    async def execute(
        self, 
        filters: dict = None,
        freshness: Freshness = Freshness.REALTIME
    ) -> ConnectorResult:
        """
        Execute query via optimal data source.
        
        The connector doesn't decide REST vs Data Lake - 
        the query router does, based on volume and freshness.
        """
        volume = await self.estimate_volume(filters)
        source = self.router.select_source(volume, freshness)
        
        if source == DataSource.REST:
            return await self._execute_rest(filters)
        else:
            return await self._execute_lake(filters)
```



## 5.2 Connector Inventory



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│                         CONNECTOR INVENTORY                                 │
│                                                                             │
│  PHASE 1 (Core)                                                             │
│  ──────────────                                                             │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ BEDROCK OPS SCHEDULER                                               │   │
│  │                                                                     │   │
│  │ Purpose:  Production schedule visibility                            │   │
│  │ Sources:  SLJobs, SLJobroutes, SLJrtSchs, SLItems, SLItemwhses,    │   │
│  │           SLWcs                                                     │   │
│  │ Volume:   300-800 operations (typical)                              │   │
│  │ Route:    REST API (real-time)                                      │   │
│  │ Tool:     get_production_schedule                                   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  PHASE 2 (Extended)                                                         │
│  ─────────────────                                                          │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ SALES ORDER TRACKER                                                 │   │
│  │ Purpose:  Open orders and backlog visibility                        │   │
│  │ Sources:  SLCos, SLCoitems, SLCustomers, SLItems                    │   │
│  │ Volume:   500-2000 orders                                           │   │
│  │ Route:    REST API (real-time)                                      │   │
│  │ Tool:     get_open_orders                                           │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ CUSTOMER SEARCH                                                     │   │
│  │ Purpose:  Find customers by name, location, etc.                    │   │
│  │ Sources:  SLCustomers, SLCustaddrs                                  │   │
│  │ Volume:   10-100 matches                                            │   │
│  │ Route:    REST API (real-time)                                      │   │
│  │ Tool:     search_customers                                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ INVENTORY STATUS                                                    │   │
│  │ Purpose:  Stock levels and availability                             │   │
│  │ Sources:  SLItems, SLItemwhses, SLItemlocs                          │   │
│  │ Volume:   100-5000 items                                            │   │
│  │ Route:    REST API (typical) / Data Lake (full catalog)             │   │
│  │ Tool:     get_inventory_status                                      │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  PHASE 3 (Analytics) - Requires Data Lake                                   │
│  ────────────────────────────────────────                                   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ ORDER HISTORY                                                       │   │
│  │ Purpose:  Historical order analysis                                 │   │
│  │ Route:    Data Lake only (bulk historical data)                     │   │
│  │ Tool:     get_order_history                                         │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ AP/AR AGING                                                         │   │
│  │ Purpose:  Accounts payable/receivable aging                         │   │
│  │ Route:    Data Lake only (financial reporting)                      │   │
│  │ Tool:     get_aging_report                                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



---



# 6. MCP Server Layer



## 6.1 What MCP Does



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│                    MCP: THE AI-READY INTERFACE                              │
│                                                                             │
│                                                                             │
│  WITHOUT MCP                           WITH MCP                             │
│  ──────────                            ────────                             │
│                                                                             │
│  AI needs to know:                     AI needs to know:                    │
│                                                                             │
│  • REST API authentication             • Tool name                          │
│  • Which 6 IDOs to call                • Parameters (typed, described)      │
│  • Property names for each             • When to use it (description)       │
│  • How to construct URLs                                                    │
│  • How to handle pagination                                                 │
│  • How to join the results                                                  │
│  • Rate limit handling                                                      │
│  • Token refresh logic                                                      │
│                                                                             │
│  ═══════════════════════════════════════════════════════════════════════   │
│                                                                             │
│  AI SEES THIS:                                                              │
│                                                                             │
│  ┌───────────────────────────────────────────────────────────────────┐     │
│  │                                                                   │     │
│  │  Tool: get_production_schedule                                    │     │
│  │                                                                   │     │
│  │  Description: Get current production schedule for Bedrock.        │     │
│  │  Use when user asks about what's being manufactured,              │     │
│  │  production status, or work center schedules.                     │     │
│  │                                                                   │     │
│  │  Parameters:                                                      │     │
│  │    • work_center (string): Filter by work center code             │     │
│  │    • job (string): Filter by job number                           │     │
│  │    • include_completed (bool): Include done operations            │     │
│  │                                                                   │     │
│  │  Returns: List of operations with job details                     │     │
│  │                                                                   │     │
│  └───────────────────────────────────────────────────────────────────┘     │
│                                                                             │
│  That's it. One tool. Three parameters. Clear description.                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



## 6.2 Tool Definitions



```python
# ═══════════════════════════════════════════════════════════════════════════
#                           MCP TOOL DEFINITIONS
# ═══════════════════════════════════════════════════════════════════════════

PRODUCTION_SCHEDULE_TOOL = Tool(
    name="get_production_schedule",
    
    description="""
    Get current production schedule for Bedrock manufacturing operations.
    
    Returns scheduled operations showing what's being manufactured, where,
    and progress status.
    
    ┌────────────────────────────────────────────────────────────────┐
    │ USE THIS WHEN:                                                 │
    │ • User asks what's being manufactured/produced                 │
    │ • User asks about production status or progress                │
    │ • User asks what's scheduled for a work center                 │
    │ • User asks about job completion percentages                   │
    │ • User asks what's running on the shop floor                   │
    └────────────────────────────────────────────────────────────────┘
    
    ┌────────────────────────────────────────────────────────────────┐
    │ DON'T USE FOR:                                                 │
    │ • Sales orders → use get_open_orders                           │
    │ • Customer info → use search_customers                         │
    │ • Inventory levels → use get_inventory_status                  │
    └────────────────────────────────────────────────────────────────┘
    
    Common work centers: WELD-01, WELD-02, PAINT-01, ASSY-01, PACK-01
    """,
    
    parameters=[
        Parameter(
            name="work_center",
            type="string",
            required=False,
            description="Work center code (e.g., 'WELD-01'). Omit for all."
        ),
        Parameter(
            name="job",
            type="string",
            required=False,
            description="Job number (e.g., 'J-12345'). Omit for all jobs."
        ),
        Parameter(
            name="include_completed",
            type="boolean",
            required=False,
            default=False,
            description="Include 100% complete operations. Default: false."
        )
    ]
)

OPEN_ORDERS_TOOL = Tool(
    name="get_open_orders",
    
    description="""
    Get open sales orders that haven't shipped yet.
    
    Returns customer orders with line items, quantities, and due dates.
    
    ┌────────────────────────────────────────────────────────────────┐
    │ USE THIS WHEN:                                                 │
    │ • User asks about open orders or backlog                       │
    │ • User asks what needs to ship                                 │
    │ • User asks about a customer's orders                          │
    │ • User asks about orders due soon                              │
    │ • User asks about late/overdue orders                          │
    └────────────────────────────────────────────────────────────────┘
    """,
    
    parameters=[
        Parameter(
            name="customer",
            type="string",
            required=False,
            description="Customer name or number. Partial match supported."
        ),
        Parameter(
            name="days_out",
            type="integer",
            required=False,
            description="Only orders due within N days. E.g., 7 for this week."
        )
    ]
)

CUSTOMER_SEARCH_TOOL = Tool(
    name="search_customers",
    
    description="""
    Search for customer information.
    
    Returns customer details including contact info and account status.
    
    ┌────────────────────────────────────────────────────────────────┐
    │ USE THIS WHEN:                                                 │
    │ • User asks about a specific customer                          │
    │ • User needs customer contact info                             │
    │ • User asks to find customers by location                      │
    │ • User needs a customer number for another query               │
    └────────────────────────────────────────────────────────────────┘
    
    NOTE: To see customer orders, first search here, then use
    get_open_orders with the customer number.
    """,
    
    parameters=[
        Parameter(
            name="query",
            type="string",
            required=True,
            description="Search term: name, number, city, or state."
        ),
        Parameter(
            name="active_only",
            type="boolean",
            required=False,
            default=True,
            description="Only active customers. False includes inactive."
        )
    ]
)

INVENTORY_STATUS_TOOL = Tool(
    name="get_inventory_status",
    
    description="""
    Get current inventory levels.
    
    Returns quantity on hand, available, and location info.
    
    ┌────────────────────────────────────────────────────────────────┐
    │ USE THIS WHEN:                                                 │
    │ • User asks about stock levels                                 │
    │ • User asks if something is in stock                           │
    │ • User asks about inventory availability                       │
    │ • User asks where items are located                            │
    └────────────────────────────────────────────────────────────────┘
    
    NOTE: For items being manufactured, use get_production_schedule.
    """,
    
    parameters=[
        Parameter(
            name="item",
            type="string",
            required=False,
            description="Item number. Omit for all (may be slow)."
        ),
        Parameter(
            name="warehouse",
            type="string",
            required=False,
            description="Warehouse code. Omit for all locations."
        ),
        Parameter(
            name="low_stock_only",
            type="boolean",
            required=False,
            default=False,
            description="Only items below reorder point."
        )
    ]
)
```



## 6.3 MCP Server Implementation



```python
# ═══════════════════════════════════════════════════════════════════════════
#                           MCP SERVER
# ═══════════════════════════════════════════════════════════════════════════

from mcp import Server
from kai_erp.connectors import (
    BedrockOpsScheduler,
    SalesOrderTracker,
    CustomerSearch,
    InventoryStatus
)

class KaiErpMcpServer(Server):
    """
    MCP Server for KAI ERP Connector.
    
    This is the layer that makes ERP data "AI-Ready".
    """
    
    def __init__(self, config: Config):
        super().__init__(
            name="kai-erp",
            version="3.0.0"
        )
        
        # Initialize shared engines
        self.rest_engine = RestEngine(config)
        self.lake_engine = DataLakeEngine(config) if config.lake_enabled else None
        
        # Register tools
        self.register_tool(PRODUCTION_SCHEDULE_TOOL, self._handle_production_schedule)
        self.register_tool(OPEN_ORDERS_TOOL, self._handle_open_orders)
        self.register_tool(CUSTOMER_SEARCH_TOOL, self._handle_customer_search)
        self.register_tool(INVENTORY_STATUS_TOOL, self._handle_inventory_status)
    
    async def _handle_production_schedule(
        self,
        work_center: str = None,
        job: str = None,
        include_completed: bool = False
    ) -> ToolResult:
        """Bridge MCP call to BedrockOpsScheduler connector."""
        
        connector = BedrockOpsScheduler(self.rest_engine, self.lake_engine)
        
        try:
            result = await connector.execute(
                filters={
                    "work_center": work_center,
                    "job": job,
                    "include_completed": include_completed
                }
            )
            
            return ToolResult(
                success=True,
                data={
                    "operations": result.data,
                    "summary": {
                        "total": len(result.data),
                        "data_source": result.source.value,
                        "query_ms": result.latency_ms
                    }
                }
            )
            
        except VolumeExceedsLimit as e:
            return ToolResult(
                success=False,
                error={
                    "message": str(e),
                    "suggestion": "Add a work_center or job filter to reduce results."
                }
            )
```



---



# 7. Bedrock Ops Scheduler (Flagship Connector)



## 7.1 Complete Specification



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│                    BEDROCK OPS SCHEDULER                                    │
│                    Flagship Connector                                       │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  PURPOSE                                                                    │
│  ───────                                                                    │
│  Provide real-time visibility into Bedrock Truck Beds production            │
│  operations. Replaces direct SQL queries from SL8.                          │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  SOURCE IDOs                                                                │
│  ───────────                                                                │
│                                                                             │
│  ┌───────────────┬─────────────────────────────────────────────────────┐   │
│  │ IDO          │ Purpose                                              │   │
│  ├───────────────┼─────────────────────────────────────────────────────┤   │
│  │ SLJobs       │ Job headers: item, qty, status                       │   │
│  │ SLJobroutes  │ Operations: work center, sequence, progress          │   │
│  │ SLJrtSchs    │ Schedule: start/finish dates per operation           │   │
│  │ SLItems      │ Item master: descriptions                            │   │
│  │ SLItemwhses  │ Inventory: qty on hand                               │   │
│  │ SLWcs        │ Work centers: descriptions                           │   │
│  └───────────────┴─────────────────────────────────────────────────────┘   │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  VOLUME PROFILE                                                             │
│  ──────────────                                                             │
│                                                                             │
│  Typical active jobs:       100-300                                         │
│  Operations per job:        3-8                                             │
│  Total operations:          300-800 (typical)                               │
│  Maximum expected:          2,000 (peak season)                             │
│                                                                             │
│  VERDICT: Well within REST API sweet spot ✅                                │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  FILTERS                                                                    │
│  ───────                                                                    │
│                                                                             │
│  • work_center: Filter to single work center (e.g., "WELD-01")             │
│  • job: Filter to single job number                                         │
│  • include_completed: Show 100% complete operations (default: false)        │
│  • date_range: Filter by schedule date (future enhancement)                 │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  OUTPUT MODEL                                                               │
│  ────────────                                                               │
│                                                                             │
│  ScheduledOperation:                                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ job: str                    # "J-12345"                             │   │
│  │ suffix: int                 # 0                                     │   │
│  │ item: str                   # "BED-KING-BLK"                        │   │
│  │ item_description: str       # "King Bed Frame - Black"              │   │
│  │ operation_num: int          # 20                                    │   │
│  │ work_center: str            # "WELD-01"                             │   │
│  │ work_center_description: str # "Welding Station 1"                  │   │
│  │ qty_released: float         # 50                                    │   │
│  │ qty_complete: float         # 23                                    │   │
│  │ pct_complete: float         # 46.0                                  │   │
│  │ sched_start: datetime       # 2024-12-09T06:00:00                   │   │
│  │ sched_finish: datetime      # 2024-12-09T14:00:00                   │   │
│  │ status: str                 # "on_track" | "behind" | "complete"    │   │
│  │ qty_on_hand: float          # 12 (inventory of finished item)       │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



## 7.2 Implementation



```python
# ═══════════════════════════════════════════════════════════════════════════
#                    BEDROCK OPS SCHEDULER CONNECTOR
# ═══════════════════════════════════════════════════════════════════════════

class BedrockOpsScheduler(BaseConnector):
    """
    Production schedule connector for Bedrock operations.
    
    This is the flagship connector that builds the foundation for all others.
    """
    
    # ─────────────────────────────────────────────────────────────────────────
    # REST API SPECIFICATION
    # ─────────────────────────────────────────────────────────────────────────
    
    def get_rest_spec(self) -> RestQuerySpec:
        return RestQuerySpec(
            idos=[
                IDOSpec(
                    name="SLJobs",
                    properties=[
                        "Job", "Suffix", "Item", "QtyReleased", 
                        "QtyComplete", "Stat", "Whse"
                    ],
                    filter="Stat='R'"  # Released jobs only
                ),
                IDOSpec(
                    name="SLJobroutes",
                    properties=[
                        "Job", "Suffix", "OperNum", "Wc",
                        "QtyComplete", "QtyScrapped"
                    ]
                ),
                IDOSpec(
                    name="SLJrtSchs",
                    properties=[
                        "Job", "Suffix", "OperNum",
                        "SchedStart", "SchedFinish"
                    ]
                ),
                IDOSpec(
                    name="SLItems",
                    properties=["Item", "Description"]
                ),
                IDOSpec(
                    name="SLItemwhses",
                    properties=["Item", "Whse", "QtyOnHand"]
                ),
                IDOSpec(
                    name="SLWcs",
                    properties=["Wc", "Description"]
                )
            ],
            
            join_sql="""
                SELECT 
                    j.Job,
                    j.Suffix,
                    j.Item,
                    j.QtyReleased,
                    j.QtyComplete as JobQtyComplete,
                    jr.OperNum,
                    jr.Wc,
                    jr.QtyComplete as OperQtyComplete,
                    jr.QtyScrapped,
                    js.SchedStart,
                    js.SchedFinish,
                    i.Description as ItemDescription,
                    iw.QtyOnHand,
                    wc.Description as WcDescription,
                    
                    -- Calculated: percent complete
                    CASE 
                        WHEN j.QtyReleased > 0 
                        THEN ROUND((jr.QtyComplete / j.QtyReleased) * 100, 1)
                        ELSE 0 
                    END as PctComplete,
                    
                    -- Calculated: status
                    CASE
                        WHEN jr.QtyComplete >= j.QtyReleased THEN 'complete'
                        WHEN js.SchedFinish < CURRENT_TIMESTAMP THEN 'behind'
                        ELSE 'on_track'
                    END as Status
                    
                FROM SLJobs j
                JOIN SLJobroutes jr 
                    ON j.Job = jr.Job AND j.Suffix = jr.Suffix
                LEFT JOIN SLJrtSchs js 
                    ON jr.Job = js.Job 
                    AND jr.Suffix = js.Suffix 
                    AND jr.OperNum = js.OperNum
                LEFT JOIN SLItems i 
                    ON j.Item = i.Item
                LEFT JOIN SLItemwhses iw 
                    ON j.Item = iw.Item AND j.Whse = iw.Whse
                LEFT JOIN SLWcs wc 
                    ON jr.Wc = wc.Wc
                    
                ORDER BY js.SchedStart, j.Job, jr.OperNum
            """
        )
    
    # ─────────────────────────────────────────────────────────────────────────
    # DATA LAKE SPECIFICATION (for bulk/historical access)
    # ─────────────────────────────────────────────────────────────────────────
    
    def get_lake_query(self) -> str:
        return """
            SELECT 
                j.job as Job,
                j.suffix as Suffix,
                j.item as Item,
                j.qty_released as QtyReleased,
                j.qty_complete as JobQtyComplete,
                jr.oper_num as OperNum,
                jr.wc as Wc,
                jr.qty_complete as OperQtyComplete,
                js.sched_start as SchedStart,
                js.sched_finish as SchedFinish,
                i.description as ItemDescription,
                iw.qty_on_hand as QtyOnHand,
                wc.description as WcDescription
                
            FROM SYTELINE.job j
            JOIN SYTELINE.jobroute jr 
                ON j.job = jr.job AND j.suffix = jr.suffix
            LEFT JOIN SYTELINE.jrt_sch js 
                ON jr.job = js.job 
                AND jr.suffix = js.suffix 
                AND jr.oper_num = js.oper_num
            LEFT JOIN SYTELINE.item i 
                ON j.item = i.item
            LEFT JOIN SYTELINE.itemwhse iw 
                ON j.item = iw.item AND j.whse = iw.whse
            LEFT JOIN SYTELINE.wc wc 
                ON jr.wc = wc.wc
                
            WHERE j.stat = 'R'
            ORDER BY js.sched_start, j.job, jr.oper_num
        """
    
    # ─────────────────────────────────────────────────────────────────────────
    # VOLUME ESTIMATION
    # ─────────────────────────────────────────────────────────────────────────
    
    TYPICAL_ACTIVE_JOBS = 200
    TYPICAL_OPS_PER_JOB = 4
    
    async def estimate_volume(self, filters: dict) -> int:
        base = self.TYPICAL_ACTIVE_JOBS * self.TYPICAL_OPS_PER_JOB  # ~800
        
        if filters:
            if filters.get("work_center"):
                return base // 10  # ~80 ops per work center
            if filters.get("job"):
                return self.TYPICAL_OPS_PER_JOB  # ~4 ops
        
        return base
    
    # ─────────────────────────────────────────────────────────────────────────
    # OUTPUT TRANSFORMATION
    # ─────────────────────────────────────────────────────────────────────────
    
    def transform_result(self, row: dict) -> ScheduledOperation:
        return ScheduledOperation(
            job=row["Job"],
            suffix=row["Suffix"],
            item=row["Item"],
            item_description=row.get("ItemDescription", ""),
            operation_num=row["OperNum"],
            work_center=row["Wc"],
            work_center_description=row.get("WcDescription", ""),
            qty_released=row["QtyReleased"],
            qty_complete=row["OperQtyComplete"],
            pct_complete=row.get("PctComplete", 0),
            sched_start=row.get("SchedStart"),
            sched_finish=row.get("SchedFinish"),
            status=row.get("Status", "unknown"),
            qty_on_hand=row.get("QtyOnHand")
        )
```



---



# 8. Implementation Roadmap



## 8.1 Phase Overview



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│                        IMPLEMENTATION TIMELINE                              │
│                                                                             │
│                                                                             │
│  Week 1        Week 2        Week 3        Week 4        Week 5            │
│    │             │             │             │             │                │
│    ▼             ▼             ▼             ▼             ▼                │
│  ┌─────────────────────────┐                                                │
│  │      PHASE 1            │                                                │
│  │   Core + Bedrock        │                                                │
│  │                         │                                                │
│  │  • REST Engine          │                                                │
│  │  • Auth/Token mgmt      │                                                │
│  │  • Parallel fetch       │                                                │
│  │  • DuckDB staging       │                                                │
│  │  • Bedrock connector    │                                                │
│  │  • FastAPI endpoints    │                                                │
│  │                         │                                                │
│  │  DELIVERABLE:           │                                                │
│  │  Production schedule    │                                                │
│  │  via REST API           │                                                │
│  └─────────────────────────┘                                                │
│                    │                                                        │
│                    ▼                                                        │
│              ┌─────────────────────────┐                                    │
│              │      PHASE 2            │                                    │
│              │   More Connectors       │                                    │
│              │                         │                                    │
│              │  • Sales Order Tracker  │                                    │
│              │  • Customer Search      │                                    │
│              │  • Inventory Status     │                                    │
│              │  • Data Lake engine     │                                    │
│              │    (if licensed)        │                                    │
│              │                         │                                    │
│              │  DELIVERABLE:           │                                    │
│              │  Full connector suite   │                                    │
│              └─────────────────────────┘                                    │
│                              │                                              │
│                              ▼                                              │
│                        ┌─────────────────────────┐                          │
│                        │      PHASE 3            │                          │
│                        │   MCP + AI Ready        │                          │
│                        │                         │                          │
│                        │  • MCP Server           │                          │
│                        │  • Tool definitions     │                          │
│                        │  • Tool handlers        │                          │
│                        │  • AI testing           │                          │
│                        │                         │                          │
│                        │  DELIVERABLE:           │                          │
│                        │  AI agents can query    │                          │
│                        │  ERP data               │                          │
│                        └─────────────────────────┘                          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



## 8.2 Phase 1 Detail: Core + Bedrock (Weeks 1-2)



```
┌─────────────────────────────────────────────────────────────────────────────┐
│ PHASE 1: CORE INFRASTRUCTURE + BEDROCK SCHEDULER                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  DAY 1-2: Project Setup                                                     │
│  ───────────────────────                                                    │
│  □ Repository initialization                                                │
│  □ Docker/Coolify configuration                                             │
│  □ Environment setup (.env, secrets)                                        │
│  □ Validate SL10 credentials via Postman                                    │
│                                                                             │
│  DAY 3-4: Authentication Layer                                              │
│  ────────────────────────────                                               │
│  □ Token acquisition from SL10                                              │
│  □ Token caching with expiry tracking                                       │
│  □ Automatic refresh at 55 minutes                                          │
│  □ Error handling (401 retry)                                               │
│                                                                             │
│  DAY 5-6: REST API Engine                                                   │
│  ────────────────────────                                                   │
│  □ HTTP client with connection pooling                                      │
│  □ Parallel fetch (asyncio.gather)                                          │
│  □ Rate limit handling (429 backoff)                                        │
│  □ Response parsing                                                         │
│                                                                             │
│  DAY 7-8: Staging Layer                                                     │
│  ─────────────────────                                                      │
│  □ DuckDB integration                                                       │
│  □ JSON → DataFrame → Table loading                                         │
│  □ SQL query execution                                                      │
│  □ Result extraction                                                        │
│                                                                             │
│  DAY 9-10: Bedrock Connector + API                                          │
│  ─────────────────────────────────                                          │
│  □ BedrockOpsScheduler connector                                            │
│  □ FastAPI endpoints                                                        │
│  □ OpenAPI documentation                                                    │
│  □ Integration testing                                                      │
│  □ Deploy to KAI server                                                     │
│                                                                             │
│  ═══════════════════════════════════════════════════════════════════════   │
│  PHASE 1 EXIT CRITERIA:                                                     │
│  ✓ GET /bedrock/schedule returns production data                            │
│  ✓ Response time < 2 seconds                                                │
│  ✓ Data matches SL8 query results (spot check 10 jobs)                      │
│  ✓ Filters work (work_center, job)                                          │
│  ✓ Container stable for 24+ hours                                           │
│  ═══════════════════════════════════════════════════════════════════════   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



## 8.3 Phase 3 Detail: MCP Integration (Week 5)



```
┌─────────────────────────────────────────────────────────────────────────────┐
│ PHASE 3: MCP SERVER + AI INTEGRATION                                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  DAY 1-2: MCP Server Framework                                              │
│  ─────────────────────────────                                              │
│  □ MCP server initialization                                                │
│  □ Tool registry                                                            │
│  □ Protocol handling                                                        │
│  □ Error translation layer                                                  │
│                                                                             │
│  DAY 3-4: Tool Implementation                                               │
│  ────────────────────────────                                               │
│  □ get_production_schedule tool                                             │
│  □ get_open_orders tool                                                     │
│  □ search_customers tool                                                    │
│  □ get_inventory_status tool                                                │
│  □ Handler functions bridging to connectors                                 │
│                                                                             │
│  DAY 5: Testing + Documentation                                             │
│  ────────────────────────────                                               │
│  □ Test with Claude Desktop                                                 │
│  □ Verify tool discovery                                                    │
│  □ Test multi-tool conversations                                            │
│  □ Document example prompts                                                 │
│  □ Create AI user guide                                                     │
│                                                                             │
│  ═══════════════════════════════════════════════════════════════════════   │
│  PHASE 3 EXIT CRITERIA:                                                     │
│  ✓ AI can discover all tools via MCP                                        │
│  ✓ "What's running in welding?" returns correct data                        │
│  ✓ Multi-step queries work (find customer → get their orders)               │
│  ✓ Errors produce helpful AI-friendly messages                              │
│  ✓ Documentation complete for AI users                                      │
│  ═══════════════════════════════════════════════════════════════════════   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



---



# 9. Appendices



## 9.1 Project Structure



```
kai-erp-connector/
├── docker-compose.yml
├── Dockerfile
├── pyproject.toml
├── README.md
│
├── src/kai_erp/
│   ├── __init__.py
│   ├── config.py
│   │
│   ├── adapters/                  # Upstream adapters (SyteLine10, legacy_*)
│   │   ├── __init__.py
│   │   ├── syteline10_cloud/
│   │   ├── legacy_global_shop/
│   │   └── legacy_syteline8/
│   │
│   ├── core/                      # Layer 1: Data Sources
│   │   ├── __init__.py
│   │   ├── auth.py               # Token management
│   │   ├── rest_engine.py        # REST API client
│   │   ├── lake_engine.py        # Data Lake client
│   │   ├── staging.py            # DuckDB staging
│   │   └── router.py             # REST vs Lake routing
│   │
│   ├── connectors/                # Layer 2: Business Logic
│   │   ├── __init__.py
│   │   ├── base.py               # BaseConnector class
│   │   ├── bedrock_ops.py        # Production schedule
│   │   ├── sales_orders.py       # Open orders
│   │   ├── customers.py          # Customer search
│   │   └── inventory.py          # Inventory status
│   │
│   ├── mcp/                       # Layer 3: AI Interface
│   │   ├── __init__.py
│   │   ├── server.py             # MCP server
│   │   ├── tools.py              # Tool definitions
│   │   └── handlers.py           # Tool handlers
│   │
│   ├── models/                    # Shared data models
│   │   ├── __init__.py
│   │   ├── operations.py
│   │   ├── orders.py
│   │   └── customers.py
│   │
│   └── api/                       # REST API (for non-AI consumers)
│       ├── __init__.py
│       ├── main.py
│       └── routes/
│           ├── bedrock.py
│           ├── sales.py
│           └── inventory.py
│
└── tests/
    ├── conftest.py
    ├── test_core/
    ├── test_connectors/
    └── test_mcp/
```



## 9.2 Environment Configuration



```bash
# .env.example

# SyteLine 10 REST API
SL10_BASE_URL=https://xxx.erpsl.inforcloudsuite.com
SL10_CONFIG_NAME=XXX_TST
SL10_USERNAME=api_user
SL10_PASSWORD=secret

# Data Lake (optional - if licensed)
LAKE_ENABLED=false
LAKE_COMPASS_URL=https://xxx.compass.infor.com
LAKE_ION_CLIENT_ID=xxx
LAKE_ION_CLIENT_SECRET=xxx

# Server
PORT=8100
LOG_LEVEL=INFO

# Volume thresholds
REST_PREFERRED_MAX=2000
REST_HARD_MAX=5000
```



## 9.3 Quick Reference



```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│                          QUICK REFERENCE                                    │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  VOLUME LIMITS                                                              │
│  • REST optimal: < 2,000 records                                            │
│  • REST maximum: < 5,000 records                                            │
│  • Data Lake: > 10,000 records                                              │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  API ENDPOINTS                                                              │
│  • GET /bedrock/schedule          Production operations                     │
│  • GET /sales/orders              Open sales orders                         │
│  • GET /customers/search          Customer lookup                           │
│  • GET /inventory/status          Stock levels                              │
│  • GET /health                    System health                             │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  MCP TOOLS                                                                  │
│  • get_production_schedule        → BedrockOpsScheduler                     │
│  • get_open_orders                → SalesOrderTracker                       │
│  • search_customers               → CustomerSearch                          │
│  • get_inventory_status           → InventoryStatus                         │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  KEY METRICS                                                                │
│  • Target response time: < 2 seconds                                        │
│  • Token refresh: 55 minutes (before 60-min expiry)                         │
│  • Rate limit: ~1000 requests/minute                                        │
│  • Data Lake sync delay: 10-15 minutes                                      │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```



---



## Document History



| Version | Date | Changes |
|---------|------|---------|
| 1.0 | Dec 2024 | Initial REST API connector proposal |
| 2.0 | Dec 2024 | Added Data Lake analysis and hybrid architecture |
| 3.0 | Dec 2024 | Complete blueprint with MCP integration |



---



*KAI ERP Connector - Making ERP Data AI-Ready*

*Kaspar Companies IT Operations*



